(ns woodcontainer.advisor
  "WoodenContainerAdvisor -- the *contained intelligence node* for the
  wooden-container-shop plant-operations coordination actor.

  It normalizes production-batch patches (dimensional-spec/unit-count/
  output-quality for crates, pallets, and barrels/casks), drafts a
  cutting/assembly-line-equipment maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag
  (materials-safety / equipment-safety / ISPM-15 heat-treatment-
  compliance), and drafts an outbound wooden-container-shipment
  ...
  a real cutting/assembly-line-equipment actuation, cutting/assembly-
  line-run execution, or freight dispatch. Every output is censored
  downstream by `woodcontainer.governor` before anything touches the
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `woodcontainer.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; assembly-line-equipment-control or
  ...
  `woodcontainer.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  validates the patch; it does not invent the batch's dimensional
  spec, unit count, output quality or verification status. High
...
(defn- flag-safety-concern
  "Draft a materials-safety/equipment-safety/ISPM-15 heat-treatment-
  compliance safety-concern flag. ALWAYS `:stake :coordination/
  ...
  this actor does NOT do` re: never blocking safety-relevant
  reporting on an administrative technicality). See
  `woodcontainer.phase`: no phase ever adds this op to a phase's
  `:auto` set; `woodcontainer.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound wooden-container-shipment coordination proposal
  against a production batch. The advisor passes through the
  caller's own claimed unit count -- it does NOT invent one, and
  `woodcontainer.governor` NEVER trusts it: it independently
  recomputes whether the batch's own cumulative-shipped unit count
  plus this claim would exceed the batch's own recorded unit count
...
(def ^:private system-prompt
  (str "あなたは木製コンテナ(クレート・パレット・樽/カスク)工場プラント運用コーディネーターの助言者です。"
       ...
       "切断・組立ラインの稼働確定(finalize)や設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `woodcontainer.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :woodcontainer-advisor-proposal
   :op         (:op request)
