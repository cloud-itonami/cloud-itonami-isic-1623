(ns woodcontainer.operation
  "WoodenContainerOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (WoodenContainerAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Wooden Container Shop Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [woodcontainer.advisor :as advisor]
            [woodcontainer.governor :as governor]
            [woodcontainer.phase :as phase]
...
(defn build
  "Compiles a WoodenContainerOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `woodcontainer.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WoodenContainerAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Wooden Container Shop Plant Operations Governor -- independent censor.
      (g/add-node :govern
