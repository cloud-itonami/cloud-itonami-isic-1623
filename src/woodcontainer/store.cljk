(ns woodcontainer.store
  "SSoT for the wooden-container-shop plant-operations coordination
  ...
  wooden-container capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             dimensional-spec/unit-count/output-quality
                             record (crates, pallets, barrels/casks).
                             ...
                             ground truth.
    - `equipment`         -- a cutting/assembly-line unit's own record
                             (crate/pallet nailing machine, stave
                             jointer for cooperage). `:verified?`/
    ...
    - `maintenance`       -- a scheduled cutting/assembly-line
                             maintenance-window DRAFT against a piece
                             of equipment (`woodcontainer.registry`'s
    ...
    - `shipments`         -- a proposed outbound wooden-container
                             shipment DRAFT (`woodcontainer.registry`'s
                             `register-shipment`).
  ...
  audit trail a shop owner or wooden-container buyer trusting this
  coordinator needs."
  (:require [woodcontainer.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (woodcontainer.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (woodcontainer.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :dimensional-spec :pallet-block-style :product-type :pallet
                :unit-count 500 :output-quality-percent 96.0
                :verified? true :registered? true
                :shipped-unit-count 100
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :dimensional-spec :crate-standard :product-type :crate
                :unit-count 80 :output-quality-percent 92.0
                :verified? true :registered? true
                :shipped-unit-count 75
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :dimensional-spec :barrel-tight-cooperage :product-type :barrel
...
(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :crate-pallet-nailer
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :stave-jointer
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `woodcontainer.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `woodcontainer.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  shipment coordinated against it); one verified+registered crate/
  pallet nailing-machine unit (schedulable for maintenance), one
  UNVERIFIED/unregistered stave-jointer unit (blocks any maintenance
