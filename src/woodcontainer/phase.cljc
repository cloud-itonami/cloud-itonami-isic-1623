(ns woodcontainer.phase
  "Phase 0->3 staged rollout for the wooden-container-shop plant-
  ...
  consequence (a cutting/assembly-line maintenance window means
  production downtime, and the equipment ends up actually touched);
  it is always a human plant supervisor's call.
  `woodcontainer.governor`'s `cutting-assembly-line-finalize-blocked-
  violations` HARD-blocks finalize attempts unconditionally, and the
  confidence/high-stakes gate independently never lets `:flag-safety-
  concern` auto-commit either -- multiple independent layers agree on
...
(defn verdict->disposition
  "Map a Wooden Container Shop Plant Operations Governor verdict to a
