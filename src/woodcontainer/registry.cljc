(ns woodcontainer.registry
  "Pure-function domain logic for the wooden-container-shop plant-
  operations coordination actor -- equipment/batch verification,
  shipment-unit-count recompute, dimensional-spec validation,
  output-quality plausibility validation, and draft maintenance-
  schedule/shipment-coordination record construction.

  Per docs/adr/0001-architecture.md Decision 1: this vertical has NO
  pre-existing `kotoba-lang/woodcontainer`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `woodcontainer.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `millwork.registry/shipment-unit-count-exceeded?`,
  `veneerpanel.registry/shipment-volume-exceeded?`): never trust a
  ...
  call to any real wooden-container-shop-operations system. It builds
  the DRAFT record a plant coordinator would keep (a scheduled
  maintenance window, a coordinated shipment), not the act of
  actuating cutting/assembly-line equipment or dispatching a real
...
(def valid-dimensional-specs
  ...
  may declare, spanning the product range ISIC 1623 covers
  (manufacture of wooden containers -- crates, pallets, and
  barrels/casks manufactured via cutting/assembly lines/cooperage): a
  standard and a heavy-duty crate dimensional class, a block-style and
  a stringer-style pallet dimensional class, and a tight-cooperage
  (liquid-tight, e.g. wine/spirits cask) and a slack-cooperage (dry
  goods, e.g. produce/hardware barrel) barrel dimensional class.
  Anything else is a fabricated/unrecognized dimensional spec -- the
  governor HARD-holds rather than let an invented spec pass through."
  #{:crate-standard :crate-heavy-duty
    :pallet-block-style :pallet-stringer-style
    :barrel-tight-cooperage :barrel-slack-cooperage})

(def output-quality-min-percent
  "Physical floor for a wooden-container batch's output-quality
...
(def output-quality-max-percent
  "Physical ceiling for a wooden-container batch's output-quality
  reading -- a yield percentage can never exceed 100% of the batch
  inspected."
  100.0)
...
(defn output-quality-valid?
  "Is `percent` a physically plausible wooden-container output-quality
...
(defn register-maintenance
  ...
  cutting/assembly-line maintenance window against a verified,
  registered piece of equipment. Pure function -- does not actuate
  cutting/assembly-line equipment or execute any maintenance; it
  builds the RECORD a plant coordinator would keep.
  `woodcontainer.governor` independently re-verifies the equipment's
  own verified/registered ground truth, and permanently blocks any
  attempt to set `:finalize? true` on a cutting/assembly-line
  schedule (see README `Actuation`), before this is ever allowed to
  commit."
...
(defn register-shipment
  ...
  outbound wooden-container shipment against a verified, registered
  production batch. Pure function -- does not dispatch any real
  freight carrier; it builds the RECORD a plant coordinator would
  keep. `woodcontainer.governor` independently re-verifies the
  shipment's own claimed unit count against `shipment-unit-count-
