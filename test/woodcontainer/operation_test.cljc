(ns woodcontainer.operation-test
  "Smoke tests for the compiled WoodenContainerOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `woodcontainer.governor-contract-test`; the Store contract in
  `woodcontainer.store-contract-test`."
...
(deftest test-actor-builds
  (testing "WoodenContainerOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :severity :moderate :description "ISPM-15 heat-treatment-compliance concern"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
