(ns woodcontainer.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control cutting/assembly-line equipment
  directly... does NOT authorize or execute maintenance/cutting-
  assembly-line operations') implemented faithfully. The single
  invariant under test:

    WoodenContainerAdvisor never schedules maintenance, flags a
    safety concern, or coordinates a shipment the Wooden Container
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:dimensional-spec :pallet-block-style}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :pallet-block-style (:dimensional-spec (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-nailer :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest cutting-assembly-line-finalize-is-held-and-permanently-blocked
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "equip-001" :maintenance-type :cutting-assembly-line-run-finalize
                             :scheduled-date "2026-09-01" :finalize? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:cutting-assembly-line-finalize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
...
(deftest invalid-dimensional-spec-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:dimensional-spec :custom-oversize-shipping-cage}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-dimensional-spec} (-> (store/ledger db) last :basis)))
    (is (not= :custom-oversize-shipping-cage (:dimensional-spec (store/batch db "batch-001"))) "fabricated dimensional spec never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "ISPM-15 heat-treatment-compliance concern near nailer"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
