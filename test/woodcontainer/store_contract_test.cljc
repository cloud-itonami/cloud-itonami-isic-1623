(ns woodcontainer.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `woodcontainer.store` ns docstring for why a second
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:dimensional-spec :crate-standard}})
    (is (= :crate-standard (:dimensional-spec (store/batch s "batch-001"))))
